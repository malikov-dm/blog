# Blog about SharePoint and Dotnet Core

Blog about SharePoint and Dotnet Core

## Index

### Git

* [Конфигурация и подготовка к работе](articles/git/git-cheat-sheet-1.md)
* [Инициализация репозитория, подключение к удаленному репозиторию, история](articles/git/git-cheat-sheet-2.md)
* [Работа с ветками](articles/git/git-cheat-sheet-3.md)
* [Отмена изменений (reset)](articles/git/git-cheat-sheet-4.md)
* [Просмотр (git diff)](articles/git/git-cheat-sheet-5.md)
* [Слияние (git merge)](articles/git/git-cheat-sheet-6.md)

### Amazon

* [Amazon Web Services](articles/amazon-web-services.md)

### Dotnet Core

* [Dotnet Core Development](articles/dotnet-core-development.md)
* [Nginx Dotnet Core](articles/nginx-dotnet-core.md)
* [Dotnet Core Reference](articles/dotnet-core-reference.md)
* [Connect Soap Service Dotnet core](articles/connect-soap-service-dotnet-core.md)

### Ubuntu

* [Installation Ubuntu on Hyper-V](articles/ubuntu/ubuntu-hyper-v-install.md)
* [Ubuntu Package Distribution](articles/ubuntu/ubuntu-package-distribution.md)

### CSharp

* [C# Sandbox](articles/csharp-sandbox.md)